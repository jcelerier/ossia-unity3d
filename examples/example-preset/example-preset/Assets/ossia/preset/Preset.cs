...
namespace Namespace {
	...
	unsafe internal class BlueYetiAPI {
	   ...
	   /// <param name="preset">A pointer to a preset that will receive the preset built from JSON.</param>
        ...
        [DllImport("ossia")]
        public static extern bool ossia_presets_has_key(IntPtr preset, string key);

        [DllImport("ossia")]
        public static extern ossia_preset_result_enum ossia_presets_key_to_string(IntPtr preset, string key, IntPtr* buffer);

        [DllImport("ossia")]
        public static extern ossia_preset_result_enum ossia_presets_key_to_value(IntPtr preset, string key, IntPtr* buffer);
	...
	unsafe public class Preset {
        ...
        internal IntPtr preset = IntPtr.Zero;
        
        public Preset(Ossia.Device dev)
        {
            if(dev != null)
            {
            fixed (IntPtr* presetptr = &preset)
            {
                var code = BlueYetiAPI.ossia_devices_make_preset(dev.ossia_device, presetptr);
                if(code != ossia_preset_result_enum.OSSIA_PRESETS_OK) {
                    throw new Exception("Error code " + code);
                }
            }
            ...
            {
                throw new Exception("Called with null device");
            }
        }
        ...
        public string GetAsString(string address)
        {
            IntPtr strptr;
            ossia_preset_result_enum code = BlueYetiAPI.ossia_presets_key_to_string(preset, address, &strptr);
            if (code == ossia_preset_result_enum.OSSIA_PRESETS_OK)
            {
                System.String str = Marshal.PtrToStringAnsi(strptr);
                BlueYetiAPI.ossia_free_string(strptr);
                return str;
            }
            else
        ...
        public Ossia.Value Get(string address)
        {
            IntPtr valueptr;
            ossia_preset_result_enum code = BlueYetiAPI.ossia_presets_key_to_value(preset, address, &valueptr);
            if (code == ossia_preset_result_enum.OSSIA_PRESETS_OK)
            {
                return new Ossia.Value(valueptr);
            }
            else
            {
                throw new Exception("Error code " + code);
